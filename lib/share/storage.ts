...
const SHARE_VIEW_TOTAL_KIND_COUNT_IDX = `${SHARE_VIEW_TOTAL_TABLE}_kind_count_idx`;
...
const TREND_24H_SOURCE = readEnv("MY9_TRENDS_24H_SOURCE") === "hour" ? "hour" : "day";
...
async function ensureSchema(): Promise<boolean> {
  ...
  if (!schemaReadyPromise) {
    schemaReadyPromise = (async () => {
      ...
      await sql`
        CREATE TABLE IF NOT EXISTS ${sql.unsafe(SHARE_VIEW_TOTAL_TABLE)} (
          share_id TEXT PRIMARY KEY,
          kind TEXT NOT NULL,
      ...
      `;
      await sql`
        CREATE INDEX IF NOT EXISTS ${sql.unsafe(SHARE_VIEW_TOTAL_KIND_COUNT_IDX)}
        ON ${sql.unsafe(SHARE_VIEW_TOTAL_TABLE)} (kind, view_count DESC, share_id)
      `;
    })();
  }
...
function getMemoryTrendSampleSummaryCache(key: string, allowExpired = false): TrendSampleSummary | null {
  const store = getMemoryStore();
...
function getBeijingHourStart(timestampMs: number): number {
...
export async function getTrendSampleSummaryCache(
  ...
  if (TREND_MEMORY_CACHE_ENABLED) {
    const fromMemory = getMemoryTrendSampleSummaryCache(key, allowExpired);
...
export async function getTrendsCache(
  ...
  if (TREND_MEMORY_CACHE_ENABLED) {
    const fromMemory = getMemoryTrendCache(key, allowExpired);
    if (fromMemory) return fromMemory;
  }
...
export async function upsertShareViewTotalCounts(
  rows: Array<{
    shareId: string;
    kind: SubjectKind;
    viewCount: number;
  ...
  const normalizedRows = rows
    ...
    .filter((row) => row.share_id && row.kind && row.view_count > 0);
  ...
  if (!sql || !(await ensureSchema())) {
    if (!MEMORY_FALLBACK_ENABLED) {
      throwDatabaseNotReady("upsertShareViewTotalCounts failed");
    }
    return 0;
  }
  ...
  try {
    const result = (await sql.query(
      `
      WITH input_rows AS (
        SELECT share_id, kind, view_count
    ...
    if (!MEMORY_FALLBACK_ENABLED) {
      throwStorageError("upsertShareViewTotalCounts failed: database write error", error);
    }
    return 0;
  }
}
