...
async function queryRolling24hTrendRows(params: {
  sql: SqlClient;
  view: TrendView;
  kind: SubjectKind;
  ...
  yearPage: TrendYearPage;
  genreExcludeCondition: string;
}): Promise<TrendSubjectCountRow[]> {
  const { sql, view, kind, fromTimestamp, overallOffset, yearPage, genreExcludeCondition } = params;
  ...
  if (view === "overall") {
    return (await sql.query(
      ...
      top_counts AS (
        ...
        ORDER BY COUNT(*) DESC, sc.subject_id ASC
      ...
      ORDER BY tc.count DESC, tc.subject_id ASC
      `,
      [kind, fromTimestamp, overallOffset]
  ...
  if (view === "genre") {
    return (await sql.query(
      ...
      subject_counts AS (
        ...
        JOIN ${SUBJECT_DIM_TABLE} d ON d.subject_id = sc.subject_id AND d.kind = $1
        WHERE sc.subject_id IS NOT NULL
        GROUP BY sc.subject_id, d.name, d.localized_name, d.cover, d.release_year, d.genres
      ),
      expanded AS (
        SELECT
          sc.subject_id,
...
export async function getAggregatedTrendResponse(params: {
  ...
  if (period === "24h") {
    const countRows = await queryRolling24hTrendRows({
      sql,
      view,
      kind,
      fromTimestamp,
